...
`go-ruby-images/images` reimplements Ruby's `images` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-images/images` directly, with no Ruby runtime;
...
Because `images` is pure Go with cgo disabled, it also compiles to **WebAssembly**
